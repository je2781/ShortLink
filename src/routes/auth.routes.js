...
router.post('/signup', (0, express_validator_1.check)('email').isEmail().withMessage('Please enter a valid E-mail!').normalizeEmail().custom((value, { req }) => {
    return user_1.default.findOne({ email: value }).then((user) => {
        if (user) {
            return Promise.reject('The E-mail is already in use');
        }
    });
}), (0, express_validator_1.body)('password', 'Password must contain at least 5 characters').trim().isLength({ min: 5 }), (0, express_validator_1.body)('c_password').trim().custom((value, { req }) => {
